...
Use short, imperative commits such as `Clarify Jellyfin compatibility`. PRs should explain the behavior change, compatibility impact, and tests run; link an issue when relevant and include screenshots for visible UI changes. Stable releases use four-part versions on `main` and a matching `vX.Y.Z.W` tag. Future beta builds come from `beta` with a `-beta` informational suffix and only the fourth version field increasing from the latest stable (for example, 1.1.0.1 after 1.1.0.0). Each beta must exceed the previous beta. The next stable version must numerically exceed every beta so Jellyfin offers an automatic update; use a higher third or second field for promotion. The beta catalog is separate.
