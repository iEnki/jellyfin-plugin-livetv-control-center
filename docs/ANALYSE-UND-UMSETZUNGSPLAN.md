...
## App-Fix — Dev 0.3.0.2 / Fire TV

Die betroffene App ist die aktuelle Jellyfin-App auf Fire TV, recherchierter offizieller Android-TV-Stand 0.19.10. Der funktionierende Browserguide wird dort nicht geladen: Der native Guide basiert auf `TvManager` und den normalen Live-TV-Sendern/Programmen. Es gibt im untersuchten Client keinen Plugin-Pages-Einstieg für die eigenständige Web-Zeitachse. Eine zweite native Rasteransicht erfordert eine Clienterweiterung; das Filtern des originalen Guides bleibt ausgeschlossen.

Der bisherige App-Kanal lieferte ausschließlich Sender-Medieneinträge. Die erwartete Gruppen-Programmansicht fehlte dort im Code vollständig. Jetzt besitzt jede Gruppe ein eigenes „Fernsehprogramm“ mit sieben Kalendertagen, Sendern, Sendungen und Beschreibungen. Ein Sendungsordner enthält einen eindeutig bezeichneten Live-Wiedergabeeinstieg; zukünftige Sendungen werden nicht als abspielbare Aufnahmen dargestellt. Zeitangaben kommen aus der konfigurierten App-Zeitzone. Tagesabfragen berücksichtigen 23/25-Stunden-Tage; neue Programmmetadaten erzeugen neue Detail-IDs, weil Jellyfin Kanal-Overview nur bei Erstellung übernimmt.

Bei der Wiedergabe bestätigte die Serverrecherche die frühere Einschränkung: `ChannelDynamicMediaSourceProvider.OpenMediaSource` wirft `NotImplementedException`. Der alte Kanalcallback umging dies, indem er Öffnen/Schließen deaktivierte und Tuner-URLs selbst probte. Dieses Verhalten ist entfernt. `GroupsMediaSourceProvider` delegiert nun an Jellyfins `LiveTvMediaSourceProvider` und gibt dessen echtes `ILiveStream` zurück. Er wird über Jellyfins Host-Exports entdeckt; der native Anbieter wird gezielt einmal darüber instanziiert. `IMediaSourceManager.OpenLiveStream` wird innerhalb des Providers nicht rekursiv aufgerufen, da dessen Sperre sonst erneut erworben würde. Die Fire-TV-App fordert auch bei normalen Videos `AutoOpenLiveStream=true` an, wodurch der neue Öffnungsvertrag nutzbar ist.

Die Quellenauflösung und Öffnung prüfen Gruppenbesitz sowie aktuell erlaubte Sender erneut. Native Live-TV-Items und andere Plugin-Kanäle erhalten von diesem Anbieter keine zusätzlichen Quellen. Bestehende Sender- und Playlist-IDs bleiben erhalten. Die Original-Senderliste und Original-EPG werden weiterhin nicht gefiltert.

Tests decken die komplette Kanalhierarchie, native Öffnung/Schließung, Exportregistrierung, fehlende Quellen-IDs, aktualisierte Details, geänderte Rechte und die Isolation anderer Nutzer/Quellen ab. Ein realer Fehlerlog des Fire-TV-Starts lag nicht vor; Hardwarewiedergabe ist deshalb nach Installation der Dev-Version zu bestätigen. Die genannten Ursachen sind nachgewiesene Code-/Vertragsprobleme, keine Behauptung eines bereits gelungenen Gerätestests.

Primärquellen: [Fire-TV-/Android-TV-Releases](https://github.com/jellyfin/jellyfin-androidtv/releases/tag/v0.19.10), [nativer Guide](https://github.com/jellyfin/jellyfin-androidtv/blob/v0.19.10/app/src/main/java/org/jellyfin/androidtv/ui/livetv/LiveTvGuideFragment.java), [Fire-TV-PlaybackInfo/AutoOpen](https://github.com/jellyfin/jellyfin-androidtv/blob/v0.19.10/app/src/main/java/org/jellyfin/androidtv/ui/playback/PlaybackManager.kt), [Kanal-Medienanbieter](https://github.com/jellyfin/jellyfin/blob/v12.0/src/Jellyfin.LiveTv/Channels/ChannelDynamicMediaSourceProvider.cs), [nativer Live-TV-Medienanbieter](https://github.com/jellyfin/jellyfin/blob/v12.0/src/Jellyfin.LiveTv/LiveTvMediaSourceProvider.cs), [Host-Exports](https://github.com/jellyfin/jellyfin/blob/v12.0/Emby.Server.Implementations/ApplicationHost.cs).
